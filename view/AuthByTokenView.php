...
class AuthByTokenView extends View
{
    ...
    private function ping3Process(int $user_id)
    {
        if ($this->request->get('utm_term') === $this->rest_api_partner::UTM_TERM) {
            $this->user_data->set($user_id, $this->user_data::PING3_VISIT_UTM, trim($this->request->get('utm_source')));
    ...
    private function checkPing3CrmAutoApprove(int $user_id, int $crm_auto_approve_order_id)
    {
        ...
        if ($crm_auto_approve_order_id && $utm_source) {
            $this->open_search_logger->create('Переписываем utm_source у заявки crm_auto_approve', [
                'user_id' => $user_id,
    ...
    private function checkPing3CrossOrder(int $user_id, int $cross_order_order_id)
    {
        // Если заявка crm_auto_approve найдена обработаем ее
        $utm_source = trim($this->request->get('utm_source'));
        if ($cross_order_order_id && $utm_source) {
            $this->open_search_logger->create('Переписываем utm_source у заявки cross_order', [
                'user_id' => $user_id,
