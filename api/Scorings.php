...
class Scorings extends Simpla
{
    ...
    public const LOG_FILE = 'scorings_api.txt';
    ...
    public const AXI_REJECT_REASONS = [
        ...
        'PRE_NOTREG_PASS'   => Reasons::REASON_LOCATION,
	...
	public function get_scorings($filter = array())
	{
        ...
        if (!empty($filter['created'])) {
            $created_filter = $this->db->placehold('AND (DATE(sc.created) = ? )', $filter['created']);
        }
    ...
    public function get_scoring_by_filter(array $filter)
    {
        $id_filter = '';
        $user_id_filter = '';
        $order_id_filter = '';
    ...
    public function getAxiRejectReason($user_id, string $factors_string)
    {
        ...
        $reason_id = (int)(self::AXI_REJECT_REASONS[$reason_key] ?? $this->reasons::REASON_UNKNOWN_AXI);
        ...
        if ($reason_id == $this->reasons::REASON_UNKNOWN_AXI) {
    ...
    {
        // проверяем скористу
        $scoring = $this->getCompleteScoringByTypes($user_id, self::APPROVED_SCORING_TYPES);
        if (!empty($scoring)) {
            return $this->getAmountByTypeFromBodyScoring((object)$scoring);
        }
    ...
    /**
     * Выбирает одобренную сумму из скоринга
     *
    ...
    /**
     * Поиск суммы из Hyper-c, по заявке
     *
     * @param int $order_id
     * @return int|string|null
     */
    public function getHyperCAmountByOrderId(int $order_id)
    {
        $sql = $this->db->placehold("SELECT approve_amount FROM s_hyper_c WHERE order_id = ? AND success = 1 AND decision = 'Approve'", $order_id);
        $this->db->query($sql);

        $amount = $this->db->result('approve_amount');

        if (!empty($amount)) {
            return $amount;
        }

        // Если в hyper-c не найдена сумма, то ищем в дополнительной таблице
        return (int)$this->order_data->read($order_id, $this->order_data::HYPER_C_APPROVE_AMOUNT);
    }
    ...
    /**
     * Должен ли клиент прохдить по скорингу Hyper C?
     * @param array|stdClass $firstLoanUser **!! Считаем, что это НК первичка**
     * @return bool
     */
    public function isHyperEnabledForUser($firstLoanUser): bool
    {
        $firstLoanUser = (array)$firstLoanUser;
        $order = $this->orders->get_last_order($firstLoanUser['id'] ?? $firstLoanUser['user_id']);
        if (!empty($order)) {
            return $this->isHyperEnabledForOrder($order);
        }

        // Если нет заявки - считаем, что это НК первичка и проверяем то, что можем
        ...
        // Только leadstech
        $userUtmSource = $firstLoanUser['utm_source'] ?: 'Boostra';
        if (!in_array($userUtmSource, $scoringType->params['utm_sources'])) {
            return false;
        }
     ...
     * @return void
     */
    public function sendSmsSuccessScorista(stdClass $scoring): void
    {
        ...
        if (!empty($sentSms)) {
            foreach ($sentSms as $sms) {
                if ($sms->type === $this->sms::TYPE_SUCCESS_SCORISTA) {
                    $this->logging(__METHOD__, '', ['user_id' => $this->user->id, 'order_id' => $scoring->order_id, 'sms_id' => $sms->id],
                                    'Смс об одобрении скористы уже был ранее отправлен пользователю', self::LOG_FILE);
                    return;
                }
            }
        }
