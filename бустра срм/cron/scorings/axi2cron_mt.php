...
class Axi2Cron_mt extends Simpla
{
    private const LOG_FILE = 'Axi2Cron_mt.txt';

	public const LOCKER_NAME = self::class;
	public const WORKERS_COUNT = 20;
    ...
    public function run()
    {
		...
		$const = 'constant';
        $sc_type = [
            'id' => $this->scorings::TYPE_AXILINK_2,
            'lock_name' => 'WAIT_AXILINK_2',
        ];
    ...
    public function handleAxiNbki($scoring)
    {
        try {
            ...
            if ($this->order_org_switch->trySwitchOrganization((int)$scoring->order_id)) {
                return;
            }

            $this->orders->priorApprove($scoring->order_id);
            $this->scorings->tryAddScoristaAndAxi_mt($scoring->order_id, 'AxiCron');
        } catch (Throwable $e) {
