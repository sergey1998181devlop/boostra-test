...
class SaveReportsInS3 extends Simpla
{
    ...
    private function getDirectory($reportType)
    {
        if ($reportType === $this->axi::SSP_REPORT) {
            $directory = ROOT . '/files/CCP/';
            $s3_directory = $this->config->s3['amp_report_url'];
        } else if ($reportType === $this->axi::CH_REPORT) {
            $directory = ROOT . '/files/credit_history/';
            $s3_directory = $this->config->s3['report_url'];
        } else {
...
$saveReportsInS3 = new SaveReportsInS3();
$saveReportsInS3->run();