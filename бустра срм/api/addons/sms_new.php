...
function send_sms_new($simpla, $phones, $message, $sender, $translit = 0, $time = 0, $id = 0, $format = 0, $query = "", $files = array())
{
    $config = _smsc_cfg($simpla, $sender);
    ...
    $m = _smsc_send_cmd("send", "cost=3&phones=".urlencode($phones)."&mes=".urlencode($message).
        ...
        ($sender === false ? "" : "&sender=".urlencode($config["provider"])).
        ($time ? "&time=".urlencode($time) : "").($query ? "&$query" : ""), $files, $config);
...
function get_status($simpla ,$id, $phone, $all = 0, $site_id)
{
    $config = _smsc_cfg($simpla, $site_id);
    $m = _smsc_send_cmd("status", "phone=".urlencode($phone)."&id=".urlencode($id)."&all=".(int)$all, $config);
...
function get_balance($simpla ,$organization)
{
    $config = _smsc_cfg($simpla, $organization);
    $m = _smsc_send_cmd("balance", "", [], $config); // (balance) или (0, -error)
...
function _smsc_send_cmd($cmd, $arg = "", $files = array(), $config = [])
{
    $url = $_url = ($config["https"] ? "https" : "http")."://smsc.ru/sys/$cmd.php?login=".urlencode($config["login"])."&psw=".urlencode($config["password"])."&fmt=1&charset=".$config["charset"]."&".$arg;
    ...
    do {
        ...
        $ret = _smsc_read_url($url, $files, 3 + $i, $config);
    }
    while ($ret == "" && $i < 5);
...
function _smsc_read_url($url, $files, $tm = 5, $config)
{
    $ret = "";
    $post = $config["post"] || strlen($url) > 2000 || $files;
    ...
    elseif ($files) {
        ...
        if (!$config["https"] && function_exists("fsockopen"))
        {
            $m = parse_url($url);
...
function _smsc_cfg($simpla, $siteID): array
{
    return [
        'login'   => $simpla->config->SMSC_LOGIN[$siteID] ?? '',
        'password'=> $simpla->config->SMSC_PASSWORD[$siteID] ?? '',
        'https'   => (bool)($simpla->config->SMSC_HTTPS[$siteID] ?? false),
        'post'    => (bool)($simpla->config->SMSC_POST[$siteID] ?? false),
        'charset' => $simpla->config->SMSC_CHARSET[$siteID] ?? 'utf-8',
        'debug'   => (bool)($simpla->config->SMSC_DEBUG[$siteID] ?? false),
        'provider'   => $simpla->config->SMSC_PROVIDER[$siteID] ?? '',
        'timeout' => 60,
        'connect_timeout' => 5,
    ];
}
