...
class Design extends Simpla
{
	...
	public function __construct()
	{
        ...
        $this->smarty->registerPlugin("modifier", "make_urls_clickable", array($this, "make_urls_clickable"));
        $this->smarty->registerPlugin("modifier", "format_phone", array($this, "format_phone_modifier"));
    ...
    public function assign_array(array $array): void
    {
	...
	public function resize_modifier($filename, $width=0, $height=0, $set_watermark=false)
	{
		...
		return $this->config->front_url.'/'.$this->config->resized_images_dir.$resized_filename_encoded.'?'.$this->config->token($resized_filename);
	}
    ...
    /*
     * Переводит URL в HTML-ссылку в тексте
     * @param string $text
     * */
    ...
    /**
     * Приводит российский номер телефона к формату +7XXXXXXXXXX
     *
     * @param string|null $phone
     * @return string
     */
    public function format_phone_modifier(string $phone = null): string
    {
        if (empty($phone)) {
            return '';
        }
