...
class LeadgidReportSiteView extends View
{
    public function fetch()
    {
        if ($daterange = $this->request->get('daterange'))
        {
            ...
            // Если site_id не передан, устанавливаем boostra по умолчанию
            if (empty($site_id)) {
                $site_id = 'boostra';
            }
            ...
            $site_condition = 'AND u.site_id = ?';
            $params = [$leadgen, $date_from, $date_to, $site_id];
            ...
            {
                $filename = 'files/reports/'.$leadgen.'_site.xls';
                ...
                $active_sheet->setCellValue('N1', 'Site ID');
                ...
                foreach ($report as $item)
                {
                    ...
                    $active_sheet->setCellValue($col++.$i, $item->site_id);
        ...
        return $this->design->fetch('leadgid_report_site.tpl');
    }
