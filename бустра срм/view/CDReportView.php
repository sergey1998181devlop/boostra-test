...
/**
 * Class CDReportView
 * Класс для выгрузки КД
 */
class CDReportView extends View
{
    ...
    {
        return $this->design->fetch('cd_report_view.tpl');
    }
    ...
    private function download()
    {
        ...
        $filename = 'files/reports/download_cd_report.xls';
        ...
        $active_sheet->setCellValue($cell++ .'1', 'id карты');
        $active_sheet->setCellValue($cell++ .'1', 'метод оплаты');
        $active_sheet->setCellValue($cell++ .'1', 'Pan card');
        ...
        foreach ($results as $item) {
            ...
            $active_sheet->setCellValue('E' . $rowNumber, $item->doc_url);
            $active_sheet->setCellValue('F' . $rowNumber, $item->amount);
            $active_sheet->setCellValue('G' . $rowNumber, $item->date_added);
            $active_sheet->setCellValue('H' . $rowNumber, $item->card_id);
            $active_sheet->setCellValue('I' . $rowNumber, $item->payment_method);
            $active_sheet->setCellValue('J' . $rowNumber, $item->pan);
            $active_sheet->setCellValue('K' . $rowNumber, $item->payment_amount);
            $active_sheet->setCellValue('L' . $rowNumber, $item->return_amount);
            $rowNumber++;
        }
    ...
    private function downloadRefund(){
        error_reporting(E_ERROR);
        ini_set('display_errors', 'On');
        ...
        $active_sheet->setCellValue($cell++ .'1', 'ФИО');
        $active_sheet->setCellValue($cell++ .'1', 'Номер телефона');
        $active_sheet->setCellValue($cell++ .'1', 'Возврат');
        $active_sheet->setCellValue($cell++ .'1', 'Дата возврата');
        ...
        $rowNumber = 2;
        foreach ($results as $item) {
            $active_sheet->setCellValue('A' . $rowNumber, $item->fio);
            $active_sheet->setCellValue('B' . $rowNumber, $item->phone_mobile);
            $active_sheet->setCellValue('C' . $rowNumber, $item->return_amount);
            $active_sheet->setCellValue('D' . $rowNumber, $item->return_date);
            $rowNumber++;
        }
