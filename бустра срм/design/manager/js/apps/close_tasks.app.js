...
console.log();            
    ...
    var _init_offers = function(){
        
        $(document).on('click', '.js-open-offer-modal', function(){
            
            $('#form_add_offer [name="task_id"]').val($(this).data('task'));
            $('#modal_add_offer').modal();            
            
        });

        $(document).on('change', '.js-offer-type-types', function(){
            var _type = $(this).val();
            
            $('.js-offer-type-values option').each(function(){
                if ($(this).val() == '' || $(this).hasClass('js-offer-type-'+_type))
                    $(this).show();
                else
                    $(this).hide();
            });
            $('.js-offer-type-values option[value=""]').attr('selected', true);
        });
        
        $(document).on('submit', '#form_add_offer', function(e){
            e.preventDefault();
            ...
            {
                Swal.fire({timer: 5000, title: '', text: 'Выберите опцию', type: 'error',});
            }
            else if ($form.find('[name=value]').val() == '')
            {
                Swal.fire({timer: 5000, title: '', text: 'Выберите размер скидки', type: 'error',});
            }
            else
            {
                $.ajax({
                    ...
                    success: function(resp){
                        ...
                        if (!!resp.success)
                        {
                            $('#modal_add_offer').modal('hide');            
                            
                            var _html = '<small>';
                            if (resp.offer.type == 'percents')
                                _html += 'Снижение процентной ставки по кредиту';
                            else if (resp.offer.type == 'amount')
                                _html += 'Увеличенная сумма займа';
                            else if (resp.offer.type == 'insure')
                                _html += 'Снижение страховки';
                            _html += '<br /><span class="label label-success">'+resp.offer.value+'</span>';
                            _html += '<small>';

                            $('.js-open-offer-modal[data-task='+resp.offer.close_task_id+']').after(_html).hide();
                            
                        }
                        else if (!!resp.error)
                        {
                            Swal.fire({timer: 5000, title: '', text: resp.error, type: 'error',});                
                        }
                        $form.removeClass('loading');
