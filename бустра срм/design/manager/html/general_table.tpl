...
{capture name='page_scripts'}
    ...
    <script>
        const ID_COLUMN = '{$id}';
        const COLUMNS = {$columns|json_encode};
    </script>
    <script src="design/{$settings->theme|escape}/js/apps/general_table.app.js" defer></script>
                                    ...
                                    {foreach $rows as $row}
                                        <tr class="js-item">
                                            {foreach $columns as $key => $column}
                                                <td>
                                                    <div class="js-visible-view js-col-{$key}">
                                                        {$row->$key|escape}
                                                    </div>
                                                    <div class="js-visible-edit" style="display: none">
                                                        <input type="text" class="form-control form-control-sm"
                                                               name="{$key}" value="{$row->$key|escape}"
                                                               {if isset($column['required']) && $column['required']}required{/if}
                                                               {if isset($column['editable']) && !$column['editable']}readonly{/if}
                                                        />
                                                    </div>
                                                </td>
                                            {/foreach}
                    ...
                    {foreach $columns as $key => $column}
                        {if !isset($column['editable']) || $column['editable']}
                            <div class="form-group">
                                <label for="{$key}" class="control-label">{$column['name']}</label>
