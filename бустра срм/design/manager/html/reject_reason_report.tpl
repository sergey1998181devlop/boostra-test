{$meta_title='Отчет по причинам отказа' scope=parent}
                        ...
                        <div id="result" class="">
                            <table class="table table-bordered table-hover">
                                ...
                                <tr>
                                    ...
                                    <th>Отказ CRM</th>
                                    <th>Отказ скористы</th>
                                    <th>Отказ акси</th>
                                    <th>Отказ андеррайтинга</th>
                                </tr>
                                </thead>
                                <tbody>
                                {if $items}
                                    {foreach $items as $item}
                                        <tr>
                                            ...
                                            <td>{$item->crm_reason}</td>
                                            <td>
                                                {if $item->scorista_ball}Балл: {$item->scorista_ball}<br>{/if}
                                                {if $item->scorista_factors}Стоп-факторы:
                                                    {foreach $item->scorista_factors as $factor}
                                                        <br>{$factor}
                                                    {/foreach}
                                                {/if}
                                            </td>
                                            <td>{$item->axi_reason}</td>
                                            <td>{$item->manager_reason}</td>
                                        </tr>
                                    {/foreach}
                                {else}
                                    <tr>
                                        <td colspan="7" class="text-danger text-center">Данные не найдены</td>
                                    </tr>
                                {/if}
                                </tbody>
                            </table>
...
<script>
    function download() {
        ...
        window.open('{$rejectedUri}?action=download&' + query, '_blank');
        return false;
    }
</script>
