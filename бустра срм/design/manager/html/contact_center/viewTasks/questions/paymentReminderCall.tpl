        ...
        <div class="form-body" id="Questions">
            ...
            <div id="customerSurvey-2" class="customerSurvey" style="">
                <div class="row">
                    <div id="question-2">
                        Добрый день {$userInfo->firstname} {$userInfo->patronymic}! Специалист 
                        {$manager->name}, организация Бустра. Разговор наш записывается.
                        Вам сегодня необходимо произвести оплату в сумме {$task->{'ОстатокОД'}+$task->{'ОстатокПроцентов'}+$task->{'ОстатокПени'}} или пролонгировать
                        Ваш займ на сумму {$task->{'СуммаДляПролонгации_Страховка'}+$task->{'СуммаДляПролонгации_Проценты'}+$task->{'СуммаДляПролонгации'}+$task->{'СуммаДляПролонгации_СМС'}}
                    </div>
                </div>
                <div class="row">
                    <div class="col" style="justify-content: center; text-align: center;">
