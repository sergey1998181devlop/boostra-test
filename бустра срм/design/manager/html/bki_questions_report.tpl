{$meta_title = 'Бюро кредитных историй' scope=parent}
...
{capture name='page_scripts'}
    ...
    <script>
        $(function () {
            ...
            let delayTimer;
            $('input.user, input.contract').on('keyup', function () {
        ...
        function download() {
            ...
            $.ajax({
                ...
                success: function (data, textStatus, jqXHR) {
                    ...
                    a.href = downloadUrl;
                    a.download = 'bki_report_' + new Date().toISOString().slice(0, 10) + '.xlsx';
...
<div class="page-wrapper">
                    ...
                    <div class="card-body">
                        ...
                        <form>
                            <div class="row">
                                ...
                                <div class="col-12 col-md-4">
                                    ...
                                    <button onclick="return window.open('{$reportUri}','_self');" type="button"
                                    ...
                                    <div class="col-12 col-md-2 py-1">
                                        ...
                                        {if $filter.type == 'select'}
                                            <select id="{$filter.name}" name="{$filter.name}"
                                                    class="form-control form-control-sm filter {$filter.name}">
                        ...
                        <div id="result" class="table-responsive">
                            <table class="table table-bordered table-hover">
                                        ...
                                        <tr data-id="{$reportRow.id}">
                                            <td>
                                                {if $can_see_client_url}
                                                    <a href="/client/{$reportRow.user_id}"
                                                       target="_blank">{$reportRow.client}</a>
                                                {else}
                                                    {$reportRow.client}
                                                {/if}
                                            </td>
                                            <td>
                                                {if $can_see_client_url}
                                                    <a href="/client/{$reportRow.user_id}"
                                                       target="_blank">{$reportRow.user_id}</a>
                                                {else}
                                                    {$reportRow.user_id}
                                                {/if}
                                            </td>
                                            <td>
                                                <a href="/order/{$reportRow.order_id}" target="_blank">
                                                    {$reportRow.contract_number}
                                                </a>
                                            </td>
                                            <td>
                                                {if $reportRow.file}
                                                    <a href="{$reportRow.file}" class="open-file-modal text-info"
                                                       data-file="{$reportRow.file}">
                                                        {$reportRow.name}
                                                    </a>
                                                {else}
                                            ...
                                            <td>
                                                {if $reportRow.created_at}
                                                    {$reportRow.created_at|date_format:'%d.%m.%Y %H:%M'}
                                                {else}
                                                    —
                                                {/if}
                                            </td>
                                            <td>{$reportRow.description|escape}</td>
                                            <td class="status-cell">
                                                <div class="d-flex align-reportRows-center">
                                                    {if in_array($reportRow.status, ['new', 'cancelled'])}
                                                        <button class="btn btn-success btn-sm mr-2"
                                                                onclick="updateStatus({$reportRow.id}, 'approve', this); return false;"
                                                                title="Одобрить">
                                                    ...
                                                    {if in_array($reportRow.status, ['new', 'approved'])}
                                                        <button class="btn btn-danger btn-sm"
                                                                onclick="updateStatus({$reportRow.id}, 'cancel', this); return false;"
                                                                title="Отклонить">
                                ...
                                {else}
                                    <tr>
                                        <td colspan="{count($reportHeaders) + 1}" class="text-center text-danger">Данные
                                            не найдены
                                        </td>
                                    </tr>
                                {/if}
    ...
    <!-- Глобальная функция для обработки ответа БКИ -->
    <script>
        function updateStatus(id, status, btn) {
            ...
            $.ajax({
                url: '{$reportUri}?action=updateStatus&id=' + id + '&status=' + status,
                method: 'GET',
                dataType: 'json',
                success: function (response) {
                    if (response.status === 'error') {
                        ...
                        // Найти строку и обновить статус
                        let row = $(btn).closest('tr');

                        // Обновить отображение статуса в столбце статуса
                        let statusBadgeCell = row.find('td:nth-child(5)'); // Столбец со статусом

                        if (status === 'approve') {
                            statusBadgeCell.html('<span class="badge badge-success">Одобрено</span>');
                            row.find('td:nth-child(6)').text(new Date().toLocaleString('ru-RU', {
                                day: '2-digit',
                                month: '2-digit',
                                year: 'numeric',
                                hour: '2-digit',
                        ...
                        let actionCell = row.find('.status-cell .d-flex');

                        if (status === 'approve') {
                            actionCell.html('<button class="btn btn-danger btn-sm" onclick="updateStatus(' + id + ', \'cancel\', this); return false;" title="Отклонить"><i class="fa fa-times"></i></button>');
                        } else if (status === 'cancel') {
                            actionCell.html('<button class="btn btn-success btn-sm mr-2" onclick="updateStatus(' + id + ', \'approve\', this); return false;" title="Одобрить"><i class="fa fa-check"></i></button>');
                        }
                    }
                },
