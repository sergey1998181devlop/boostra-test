{$meta_title='Отчёт по анализу email переписки' scope=parent}
...
{capture name='page_scripts'}
    ...
    <script>
        $(function () {
            ...
            let delayTimer;
            $('input.user, input.email, input.ticket').on('keyup', function() {
        ...
        function download() {
            ...
            $.ajax({
                ...
                success: function (data, textStatus, jqXHR) {
                    ...
                    a.href = downloadUrl;
                    a.download = 'usedesk_ticket_analysis_report_' + new Date().toISOString().slice(0, 10) + '.xlsx';
                                                ...
                                                {if $header.key == 'client'}
                                                    <td>
                                                        {if $can_see_client_url}
                                                            <a href="/client/{$row.client_id|escape}" target="_blank">{$row.client|escape}</a>
                                                        {else}
                                                            {$row.client|escape}
                                                        {/if}
                                                    </td>
                                                {elseif $header.key == 'ticket'}
                                                    <td>
                                                        <a href="{$row.ticket}" target="_blank">{$row.ticket_id}</a>
                                                    </td>
                                                {else}
                                                    <td>
