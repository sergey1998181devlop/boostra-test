{$meta_title='Кредитный рейтинг "Почему отказ"' scope=parent}
        ...
        <div class="row page-titles">
            <div class="col-md-6 col-8 align-self-center">
                <h3 class="text-themecolor mb-0 mt-0">
                    ...
                    <span>Кредитный рейтинг "Почему отказ"</span>
                </h3>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="/">Главная</a></li>
                    <li class="breadcrumb-item active">Кредитный рейтинг "Почему отказ"</li>
                </ol>
            </div>
        </div>
        <div class="row">
            <div class="col-12">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="text-danger">Данные достоверны с 30.01.2023</p>
                        <div id="result">
                                ...
                                {if $metric}
                                    {foreach $metric as $date => $metric_row}
                                        <tr>
                                            ...
                                            <td>{$metric_row.items.link_viewed}</td>
                                            <td>{$metric_row.items.link_click}</td>
                                            <td>{$metric_row.items.get_rating}</td>
                                            <td>{$metric_row.items.get_code}</td>
                                            <td>{$metric_row.items.reg_code}</td>
                                            <td>{$metric_row.items.get_pay}</td>
                                            <td>{$metric_row.items.total_transactions}</td>
                                            <td>{$metric_row.items.total_fact_pay}</td>
                                        </tr>
                                        <tr class="small">
                                            <td class="text-warning"></td>
                                            <td class="text-warning">{$metric_row.cv.link_click}</td>
                                            <td class="text-warning">{$metric_row.cv.get_rating}</td>
                                            <td class="text-warning">{$metric_row.cv.get_code}</td>
                                            <td class="text-warning">{$metric_row.cv.reg_code}</td>
                                            <td class="text-warning">{$metric_row.cv.get_pay}</td>
                                            <td class="text-warning">{$metric_row.cv.total_transactions}</td>
                                            <td class="text-warning"></td>
                                        </tr>
                                    {/foreach}
                                    {generateTotals totals=$totals}
