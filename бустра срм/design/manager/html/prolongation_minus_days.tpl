{* Нужно рефакторить, функционал практически аналогичен cc_prolongations.tpl *}
{$meta_title='Мои задачи' scope=parent}

{capture name='page_scripts'}
    <script type="text/javascript" src="design/{$settings->theme|escape}/js/apps/prolongations_minus_days.js?v=1.001"></script>
    ...
    <script>
        ...
        async function getSmsToUsers() {
            try {
                ...
                let resp = await $.ajax({
                    url: '/ajax/get_sms_report.php?period=minus&date='+ dataRange,
                    type: 'GET'
                });
        ...
        //             data: { date: date, managers: checkedValues},
        //             success: function(resp){
        //                 Swal.fire({
        //                     timer: 5000,
        ...
        //             type: 'POST',
        //             url:'ajax/insert-schedule.php',
        //             data: { date: date, managers: managers},
        //         })
        //
        //             .then(function () {
        //                 $.ajax({
        ...
        function EditManager() {
            ...
            if (content.find('.vox-inputs').length > 0) {
                ...
                $.ajax({
                    type: 'GET',
                    url: 'ajax/get_company_managers.php?minus=true&bool=true',
                    success: function (resp) {
                ...
                <select multiple  class="form-control  js-vox-manager" id="js-vox-manager" name="manager_id" placeholder = "Менеджеры">
                    ...
                    <div>
                      ...
                      <select class='btn btn-light period'>
                        <option value='-5'>Минус 5</option>
                        <option value='-4'>Минус 4</option>
                        <option value='-3'>Минус 3</option>
                        <option value='-2'>Минус 2</option>
                        <option value='-1'>Минус 1</option>
                       </select>
                    </div>
            </div>
        ...
        $(document).on('click','.downloadCallList',function () {
            ...
            $.ajax({
                ...
                data: { dataRange: dataRange, manager: manager},
                success:function (resp){
                    $('.preloader').hide();
                    console.log(resp)
        ...
        <div class="row page-titles">
            <div class="col-md-2 col-10 align-self-center">
                <h3 class="text-themecolor mb-0 mt-0">
                    ...
                    <span>Продление минусовые дни</span>
                    <br />
            ...
            <div class="col-md-1 pt-4">
                {if 0 && $manager->role == 'robot_minus'}{*убрал кнопку*}
                ...
                {elseif $manager->id == 77 || $manager->id == 89 || $manager->id == 144 || in_array($manager->role, ['developer', 'admin', 'opr', 'ts_operator'])}
                    <button type="button" class="btn btn-primary js-schedule-open employs-edit">Редактировать </button>
                {/if}
            </div>
            <div class="col-md-3">
                <div class="p-2">
            ...
            <div class="col-md-5 offset-md-1 col-4 align-self-center">
                ...
                <div class="row bg-grey js-filter-diagrams">
                    <div class="col-md-12">
                    ...
                    <div class="col-md-4">
                        <div class="card m-0  bg-grey">
                            <div class="card-body p-0 pt-2">
                                <div class="row">
                                    <div class="col-4">
                                        <div data-label="{$statistic->clients_amount}" class="css-bar css-bar-sm mb-0 css-bar-success css-bar-100"></div>
                                    </div>
                                    <div class="col-8">
                                        <h5 class="card-title mb-1">Кол-во клиентов</h5>
                                        <h6 class="text-white">
                                            {$statistic->clients_amount}
                                        </h6>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="col-md-3 col-10 align-self-center">
                <ol class="breadcrumb w-">
                    ...
                    <li class="breadcrumb-item active">Продление минусовые дни</li>
                </ol>
            </div>
        </div>
        <!-- ============================================================== -->
                    ...
                    <div class="card-body">
                        <div class="clearfix js-filter-status">
                            <h4 class="card-title float-left">
                                Вкладки
                                {if $filter_period=='-1'}(1 день до просрочки){/if}
                                {if $filter_period=='-2'}(2 дня до просрочки){/if}
                                {if $filter_period=='-3'}(3 дня до просрочки){/if}
                                {if $filter_period=='-4'}(4 дня до просрочки){/if}
                                {if $filter_period=='-5'}(5 дней до просрочки){/if}
                                {if $filter_period=='all'}(всего){/if}
                            </h4>
                            <div class="float-left ml-5">
                                <a href="{url period='-5' page=null}" class="btn btn-xs {if $filter_period=='-5'}btn-success{else}btn-outline-success{/if}">Минус 5 дней</a>
                                <a href="{url period='-4' page=null}" class="btn btn-xs {if $filter_period=='-4'}btn-info{else}btn-outline-info{/if}">Минус 4 дня</a>
                                <a href="{url period='-3' page=null}" class="btn btn-xs {if $filter_period=='-3'}btn-warning{else}btn-outline-warning{/if}">Минус 3 дня</a>
                                <a href="{url period='-2' page=null}" class="btn btn-xs {if $filter_period=='-2'}btn-danger{else}btn-outline-danger{/if}">Минус 2 дня</a>
                                <a href="{url period='-1' page=null}" class="btn btn-xs {if $filter_period=='-1'}btn-danger{else}btn-outline-primary{/if}">Минус 1 день</a>
                            </div>
                            <div class="float-right">
                                ...
                                {if in_array($manager->role, ['developer', 'admin', 'opr', 'ts_operator'])}
                                    <form
                                        ...
                                        <div class="check_box_block">
                                            <input type="checkbox" class="checkbox-mis color1 d-none" id="min5" name="min5">
                                            <label class="labels-missing color1" for="min5">-5</label>
                                            <div class="input_block">
                                                <input type="text" name="min5_comp" class="form-control input-sm" placeholder="ID" value="{$settings->ccprolongations5|escape}">
                                            </div>
                                        </div>
                                        <div class="check_box_block">
                                            <input type="checkbox" class="checkbox-mis color1 d-none" id="min4" name="min4">
                                            <label class="labels-missing color1" for="min4">-4</label>
                                            <div class="input_block">
                                                <input type="text" name="min4_comp" class="form-control input-sm" placeholder="ID" value="{$settings->ccprolongations4|escape}">
                                            </div>
                                        </div>

                                        <div class="check_box_block">
                                            <input type="checkbox" class="checkbox-mis color2 d-none" id="min3" name="min3">
                                            <label class="labels-missing color2" for="min3">-3</label>
                                            <div class="input_block">
                                                <input type="text" name="min3_comp" class="form-control input-sm" placeholder="ID" value="{$settings->ccprolongations3|escape}">
                                            </div>
                                        </div>
                                        <div class="check_box_block">
                                            <input type="checkbox" class="checkbox-mis color2 d-none" id="min2" name="min2">
                                            <label class="labels-missing color2" for="min2">-2</label>
                                            <div class="input_block">
                                                <input type="text" name="min2_comp" class="form-control input-sm" placeholder="ID" value="{$settings->ccprolongations2|escape}">
                                            </div>
                                        </div>
                                        <div class="check_box_block">
                                            <input type="checkbox" class="checkbox-mis color3 d-none" id="min1" name="min1">
                                            <label class="labels-missing color3" for="min1">-1</label>
                                            <div class="input_block">
                                                <input type="text" name="min1_comp" class="form-control input-sm" placeholder="ID" value="{$settings->ccprolongations1|escape}">
                                            </div>
                                        </div>
                                        <button type="submit" class="btn btn-sm btn-secondary" onclick="" title="Отправить в VOX">
                                ...
                                <div class=" input-group mb-3">
                                    ...
                                    <button type="button" class="btn btn-sm btn-primary js-blacklist-call-user input-group-append">
