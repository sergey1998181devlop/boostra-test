{$meta_title='Воронка займы ПК смс' scope=parent}
        ...
        <div class="row page-titles">
            <div class="col-md-6 col-8 align-self-center">
                <h3 class="text-themecolor mb-0 mt-0">
                    ...
                    <span>Воронка займы ПК смс</span>
                </h3>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="/">Главная</a></li>
                    <li class="breadcrumb-item active">Воронка займы ПК смс</li>
                </ol>
            </div>
        </div>
        <div class="row">
            <div class="col-12">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <h4 class="text-danger animate-flashing">Внимание! Минимальная дата для отчёта 2023.08.24</h4>
                        <form id="report_form">
                            <div class="row">
...
{*                                            <button class="btn btn-block btn-secondary dropdown-toggle" id="dropdown-button-utm_sources" type="button" data-toggle="dropdown" aria-haspopup="false" aria-expanded="true" data-flip="false">*}
{*                                                Источники*}
{*                                            </button>*}
{*                                            <div class="dropdown-menu p-2" id="dropdown-utm_sources">*}
{*                                                {include 'html_blocks/filter_utm_sources.tpl'}*}
{*                                            </div>*}
{*                                        </div>*}
{*                                    </div>*}
{*                                </div>*}
...
{capture name='page_scripts'}
    ...
    <script>
        ...
        $('.daterange').on('apply.daterangepicker', function() {
            let filter_date_start = $(this).data('daterangepicker').startDate.format('YYYY-MM-DD'),
                filter_date_end = $(this).data('daterangepicker').endDate.format('YYYY-MM-DD');

            $("#report_form").addClass('data-loading');

            {*$("#dropdown-utm_sources").load('/ajax/report_helper.php?action=getUtmSourcesByDate', {literal}{filter_date_start, filter_date_end}{/literal}, function (response, status, xhr) {*}
            ...
            {*    }*}
            {*});*}
            $("#dropdown-webmaster_id").load('/ajax/report_helper_sms.php?action=getWebmasterIdByFilter',  function () {
                    $("#report_form").removeClass('data-loading');
                if (status === "error") {
                    alert(xhr.status + " " + xhr.statusText);
                                console.error(xhr.status + " " + xhr.statusText);
        ...
        {*$(document).on('change', '[name="filter_utm_source[]"]', function (e) {*}
        ...
        {*        filter_date_start = $('.daterange').data('daterangepicker').startDate.format('YYYY-MM-DD'),*}
        {*        filter_date_end = $('.daterange').data('daterangepicker').endDate.format('YYYY-MM-DD'),*}
        {*        filter_date_accept = {*}
        {*            filter_date_start,*}
        {*            filter_date_end*}
        {*        },*}
        {*        filter_utm_sources = [$_checked_input.val()];*}

        {*    if (!is_all_checked && total_utm_source === 1) {*}
        {*        $_button.prop('disabled', false);*}
        {*        $("#report_form").addClass('data-loading');*}
        {*        $("#dropdown-webmaster_id").load('/ajax/report_helper.php?action=getWebmasterIdByFilter', {literal}{filter_date_accept, filter_utm_sources}{/literal}, function () {*}
        {*            $("#report_form").removeClass('data-loading');*}
        {*        });*}
