{$meta_title='Отчёт по анализу звонков ИИ' scope=parent}
    ...
    <script>
        $(function () {
            ...
            function updateFilters() {
                const paramsObj = {
                    user: $('input.user').val() || '',
                    phone_mobile: $('input.phone_mobile').val() || '',
                    operators: $('select.operators').val() || '',
                    tags: $('select.tags').val() || '',
                    assessment: $('select.assessment').val() || '',
                    menu: $('select.menu').val() || '',
                    conversation_topics: $('select.conversation_topics').val() || '',
                    type_report: $('select.type_report').val() || '',
            ...
            let delayTimer;
            $('input.user, input.phone_mobile').on('keyup', function() {
        ...
        function download() {
            ...
            $.ajax({
                ...
                success: function (data, textStatus, jqXHR) {
                    ...
                    a.href = downloadUrl;
                    a.download = 'analysis_calls_report_' + new Date().toISOString().slice(0, 10) + '.xlsx';
        ...
        function openTranscribeModal(element, columnTitle) {
            document.getElementById('transcribeModalLabel').innerText = columnTitle;
            document.getElementById('transcribeModalBody').innerText = element.textContent || element.innerText;
                                    ...
                                    {foreach from=$reportRows item=row}
                                        <tr>
                                            {foreach from=$reportHeaders item=header}
                                                ...
                                                {elseif $header.key == 'client'}
                                                    <td>
                                                        {if $can_see_client_url}
                                                            <a href="/client/{$row.client_id|escape}">{$row.client|escape}</a>
                                                        {else}
                                                            {$row.client|escape}
                                                        {/if}
                                                    </td>
                                                {else}
                                                    <td>
                                                        <div class="limited-text" onclick="openTranscribeModal(this, '{$header.label}')"
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="transcribeModalLabel">Содержимое</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Закрыть">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body" id="transcribeModalBody" style="max-height: 50vh; overflow-y: auto;color: #fff;">
