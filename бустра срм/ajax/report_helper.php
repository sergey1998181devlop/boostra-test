...
class ReportHelper extends Simpla {
    ...
    {
        $filter_utm_sources = $this->request->post('filter_utm_sources') ?: [];
        $filter_date_accept = $this->request->post('filter_date_accept') ?: [];

        $webmaster_ids = $this->orders->getWebmasterIdsByFilter(
            compact(
                'filter_utm_sources',
                'filter_date_accept',
            )
