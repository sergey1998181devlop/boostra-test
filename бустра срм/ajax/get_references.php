...
class GetReferences extends Simpla
{
    ...
    public function run()
    {
        ...
        if ($loanId && $type && $type == 'additional') {
            $response = $this->soap->contractForms($loanId);
            if ($response && !isset($response['error']) && !empty($response['return'])) {
                if ($responseArr = json_decode($response['return'], true)) {
                    $response['return'] = $responseArr['СправкаДопУслуги'];
                }
            }
        }
